...
# Usage
Go to Terminal/Command Prompt and run one of the following commands:

Single mode (one file at a time):
```
...
```
python3 srt-to-praat.py batch srt_di media_input output -d -c
```
## Arguments
`srt_input`: Path to the input SRT file.

`media_input`: Path to the input media file. It serves to determine the total duration of the output .TextGrid file. A wide range of media formats are supported.
